...
{
    return (a <= (double)0.0)? -a:a;    
//     return (-(a<0.0)+(a>=0.0))*a;
}

// inline double fastabs(float f) {
//     int i=((*(int*)&f)&0x7fffffff);
//     return (double)(*(float*)&i);
// }        
// 
// inline double fastabs(double f){
//     return (uint64 *)&f &= ~(1ULL << 63);
// }
...
int neiE1(double *x, int i, double scal, int bs, double eps, int *box, int *lis, int *mxi) {
	...
	mi = box[mp];
	while (mi >= 0) {
		dd = x[mi] - xc; if (fastabs(dd) <= eps) nx++;
		mi = lis[mi];
	}
	if (mm >= mp) return nx - 1;
	mi = box[mm];
	while (mi >= 0) {
		dd = xc - x[mi]; if (fastabs(dd) <= eps) nx++;
		mi = lis[mi];
	}
	nx += mxi[mp - 1] - mxi[mm];
	return nx - 1;
	...
	while (eps>epsgrid*(jj - 1)) {
		...
		for (ix1 = ix - jj; ix1 <= ix + jj; ix1++) {
			...
			for (iy1 = iy - jj; iy1 <= iy + jj; iy1 += step) {
				el = box[ix2][iy1&ib];
				while (el != -1) {
					dd = fastabs(xx[0] - x[0][el]);
					for (d = 1; d<dim; d++) 
						if ((dy = fastabs(xx[d] - x[d][el]))>dd)
							if ((dd = dy) > eps)
								break;
					if (dd <= eps)
						nx++;
		...
		for (ix1 = ix - jj; ix1 <= ix + jj; ix1 += step) {
			...
			for (iy1 = iy - jj + 1; iy1 <= iy + jj - 1; iy1++) {
				...
				while (el != -1) {
					dd = fastabs(xx[0] - x[0][el]);
					for (d = 1; d<dim; d++) if ((dy = fastabs(xx[d] - x[d][el]))>dd) if ((dd = dy) > eps) break;
					if (dd <= eps) nx++;
					el = lis[el];
				}
			}
	...
	while (eps[K - 1] > epsgrid*(jj - 1)) {
		...
		for (ix1 = ix - jj; ix1 <= ix + jj; ix1++) {
			...
			for (iy1 = iy - jj; iy1 <= iy + jj; iy1 += step) {
				...
				while (el != -1) {
					dd = fastabs(xx[0] - x[0][el]);
					for (d = 1; d<dim; d++) if ((dy = fastabs(xx[d] - x[d][el]))>dd) if ((dd = dy) > eps[K - 1]) break;;
					for (ik = 0; ik < K; ik++) {
						if (dd <= eps[ik]) nx[ik]++;
					}
					el = lis[el];
		...
		for (ix1 = ix - jj; ix1 <= ix + jj; ix1++) {
			...
			for (iy1 = iy - jj; iy1 <= iy + jj; iy1 += step) {
				...
				while (el != -1) {
					if (el != i) {
						dd = fastabs(xx[0] - x[0][el]);
						for (d = 1; d<dim; d++) if ((dy = fastabs(xx[d] - x[d][el]))>dd) dd = dy;
						if (dd < dn[K]) {
			...
			for (iy1 = iy - jj + 1; iy1 <= iy + jj - 1; iy1++) {
				...
				while (el != -1) {
					if (el != i) {
						dd = fastabs(xx[0] - x[0][el]);
						for (d = 1; d<dim; d++) if ((dy = fastabs(xx[d] - x[d][el]))>dd) dd = dy;
						if ((dy = fastabs(xx[1] - x[1][el])) > dd) dd = dy;
						if (dd < dn[K]) {
							k = K; while (dd < dn[k]) { if (k < K) { dn[k + 1] = dn[k]; nn[k + 1] = nn[k]; } k--; }
							dn[k + 1] = dd; nn[k + 1] = el;
						}
...
void mi2(double **x, int N, int K,
	...
	for (i = 1; i <= K; i++)
		phi[i] = psi[i] - (double)(dim - 1) / (double)(i);   // 
	...
	for (i = 0; i < N; i++) {
		...
		for (d = 0; d < dim; d++) {
			eps[d] = 0;
			for (k = 1; k <= K; k++) { if ((dx = fastabs(xc[d] - x[d][nn[k]])) > eps[d]) eps[d] = dx; }
...
void mi2r(double **x, int N, int K,
	...
	phi = (double*)calloc(K + 1, sizeof(double));
	for (i = 1; i <= K; i++) phi[i] = psi[i] - (double)(dim - 1) / (double)(i);   // 
...
void redr_embed(double **x, int dim, int edim, int tau, int N, int K,
	...
	for (i = 0; i < N_; i++) {
		...
		for (d = 0; d < dim; d++) {
			...
			for (de = 0; de < edim; de++)
				for (k = 1; k <= K; k++)
					if ((dx = fastabs(xc[d*edim + de] - xx[d*edim + de][nn[k]])) > eps[d]) eps[d] = dx;
		}
		for (d = 0; d < dim; d++) {
...
void mi_xnyn(double **x, int dimx, int dimy, int N, int K,
	...
	int **boxy = 0;
	int **boxx = 0;
	int *lis; // two dimensional boxes
	
	int *lisy=0;
	int *lisx=0; // lists for two dimensions
	
	int *boxx1=0;
	int *boxy1 = 0; // onedimensional boxes
	int *lisy1 = 0; 
	int *lisx1 = 0; // lists for one dimensions

	int *mxi = 0;
	int *myi=0; //accumulative lists of points in oned boxes
	...
	phi = (double*)calloc(K + 1, sizeof(double));
	for (i = 1; i <= K; i++) phi[i] = psi[i] - 1.0 / (double)i;   // 
	...
	if (dimx > 1) {
		...
		boxx1 = (int*)calloc(BOX1 + 1, sizeof(int));
	...
	if (dimy > 1) {
		...
		boxy1 = (int*)calloc(BOX1 + 1, sizeof(int));
	...
	for (i = 0; i < N; i++) {
		...
		epsx = 0; for (d = 0; d < dimx; d++)
			for (k = 1; k <= K; k++) if ((dx = fastabs(xc[d] - x[d][nn[k]])) > epsx) epsx = dx;
		epsy = 0; for (d = dimx; d < dimx + dimy; d++)
			for (k = 1; k <= K; k++) if ((dy = fastabs(xc[d] - x[d][nn[k]])) > epsy) epsy = dy;
...
void mir_xnyn(double **x, int dimx, int dimy, int N, int K,
	...
	int *lisy1=0;
	int *lisx1=0; // lists for one dimensions
	int *mxi = 0 , *myi = 0; //accumulative lists of points in oned boxes
	int *ind, *indx, *indy; //indeces of original data (the data resorted during box creating)
	double epsilon;
	int inveps;
...
void redK(double **x, int dim, int N, int K,
	...
	for (i = 0; i < N; i++) {
		...
		for (ik = 0; ik < K; ik++) {
			...
			for (d = 0; d < dim; d++) {
				eps[ik][d] = 0;
				for (k = 1; k <= ik + 1; k++) { if ((dx = fastabs(xc[d] - x[d][nn[k]])) > eps[ik][d]) eps[ik][d] = dx; }
...
void mi_xnynK(double **x, int dimx, int dimy, int N, int K,
	...
	int BOX, BOX1;
	int **box, **boxy = 0, **boxx = 0, *lis; // two dimensional boxes
	int *lisy = 0, *lisx = 0; // lists for two dimensions
	int *boxx1 = 0, *boxy1 = 0; // onedimensional boxes
	int *lisy1=0, *lisx1 = 0; // lists for one dimensions
	int *mxi = 0, *myi=0; //accumulative lists of points in oned boxes
	int *ind, *indx, *indy; //indeces of original data (the data resorted during box creating)
	double epsilon;
	int inveps;
	...
	for (i = 0; i<N; i++) {
		...
		for (ik = 0; ik < K; ik++) {
			epsx[ik] = 0; for (d = 0; d < dimx; d++)
				for (k = 1; k <= ik + 1; k++) if ((dx = fastabs(xc[d] - x[d][nn[k]])) > epsx[ik]) epsx[ik] = dx;
			epsy[ik] = 0; for (d = dimx; d < dimx + dimy; d++)
				for (k = 1; k <= ik + 1; k++) if ((dy = fastabs(xc[d] - x[d][nn[k]])) > epsy[ik]) epsy[ik] = dy;
...
void mi_xnynKembed(double **x, int dim, int N, int K,
	...
	for (i = 0; i < N; i++) {
		...
		for (ik = 0; ik<K; ik++) {
			maxdim[ik] = 0;
			if (epsx[ik]>epsy[ik]) { Eps[ik] = epsx[ik]; }
			else { Eps[ik] = epsy[ik]; maxdim[ik] = 1; }
		}
...
void mi2h(double **x, int N, int K,
	...
	for (i = 1; i <= N; i++) phi[i] = psi[i] - (dim - 1.0) / (double)i;   // 
...
void mi2r_(double **x, int N, int K,
	...
	phi = (double*)calloc(K + 1, sizeof(double));
	for (i = 1; i <= K; i++) phi[i] = psi[i] - (dim - 1.0) / (double)i;   // 
